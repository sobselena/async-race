...
export class CarsView extends Component {
  ...
  createControlButtons(id: number): Component {
    ...
    const startButton = new Button({
      ...
      text: 'Start',
      onClick: () => {
        this.events.START(id).catch(console.error);
      },
    });
    if (carData) {
  ...
  moveCar(id: number, velocity: number, distance: number) {
    ...
    const car = this.carsMap.get(id);
    if (!car || car.state !== carStates.MOVING) return;
    ...
    const transitionEndCallback = () => {
      ...
      car.state = carStates.FINISHED;
      car.stateContainer?.setText(carStates.FINISHED);
    };
    car.carImgWrapper.addListener('transitionend', transitionEndCallback);
  }

  stopCar(id: number, result: CarStatesValues) {
    const car = this.carsMap.get(id);
    if (!car) return;
    ...
    car.stateContainer?.setText(result);
    car.state = result;
  }
